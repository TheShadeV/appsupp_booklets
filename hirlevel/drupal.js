(async () => {
    ...
    function parseXml(xmlText) {
        ...
        if (error) {
            throw new Error(
                "A DOCX egyik XML fájlja nem olvasható."
            );
        }
    ...
    function readPageMetrics(
        ...
        let marginRightTwips =
            1440;

        let marginTopTwips =
            1440;

        let marginBottomTwips =
            1440;
    ...
    function getEffectiveContainerWidthTwips(
        context
    ) {
        let width =
        ...
        const paragraph =
            context.currentParagraphProperties ||
            {};
    ...
    function paragraphPropertiesToCss(
        properties
    ) {
        const styles = {
            margin: "0",
    ...
    async function readStyleSystem(
        zip,
        themeFonts
    ) {
        const file =
    ...
    async function readNumberingSystem(
        zip,
        themeFonts
    ) {
        const result = {
    ...
    function listStyleFromNumbering(
        listInfo
    ) {
        const format =
            listInfo
    ...
    function renderListItems(
        ...
        for (
            ...
            if (
                ...
                html +=
                    `<li style="${escapeAttribute(cssString(item.listCss))}">`;
    ...
    function getDrupalCsrfToken() {
        ...
        throw new Error(
            "Nem található az aktuális _csrf token."
        );
    }
    ...
    function getCkeditorCsrfToken() {
        ...
        throw new Error(
            "Nem sikerült lekérni az aktuális CKEditor CSRF tokent."
        );
    }
    ...
    async function uploadDocumentImages(
        ...
        for (
            ...
            if (!imageFile) {
                console.warn(
                    "A DOCX-ben hivatkozott kép nem található:",
                    zipPath
                );
            ...
            progress?.(
                `Kép feltöltése ${index + 1}/${ids.length}: ${filename}`
            );
    ...
    // =========================================================
    // IMAGE DIMENSIONS / TABLE-AWARE IMAGE LOGIC
    // =========================================================

    function getDrawingRelationId(
        drawing
    ) {
        const blip =
            firstDescendant(
    ...
    function analyzeTableRow(
        ...
        const imageCount =
            countImagesInElement(
                row
            );

        const hasText =
            hasRealTextInElement(
                row
            );
        ...
        if (tblW) {
            ...
            if (
                ...
                if (
                    ...
                    const base =
                        getEffectiveContainerWidthTwips(
                            context
                        );
    ...
    function calculateWidthBasedImagePercent(
        drawing,
        context
    ) {
        const dimensions =
        ...
        const containerWidthTwips =
            getEffectiveContainerWidthTwips(
                context
            );

        if (
            !dimensions.widthTwips ||
            !containerWidthTwips
        ) {
            return 100;
        }
    ...
    function getHeightReferenceCss(
        drawing,
        context
    ) {
        const dimensions =
        ...
        /*
         * Ha a Word szerinti méret túl széles a cellához,
         * inkább szélességre zsugorítjuk.
         * Így biztosan nem torzul.
         */
        if (
            widthPt >
            containerWidthPt
         ...
         * - kép + szöveg
         *
         * Itt a Wordben lévő magasság a referencia.
         * width:auto tartja a képarányt.
         */
        return {
            width: "auto",
    ...
    function drawingToHtml(
        drawing,
        context
    ) {
        const relationId =
            getDrawingRelationId(
                drawing
            );
         ...
         *
         * Táblázaton kívül:
         *   -> Word szövegtörzs szélessége referencia
         */
        if (
            context
                .currentTableRowAnalysis
                ?.useHeightReference
        ) {
            Object.assign(
                styles,
                getHeightReferenceCss(
                    drawing,
                    context
                )
            );
            ...
            const percent =
                calculateWidthBasedImagePercent(
                    drawing,
                    context
                );
    ...
    function getEffectiveRunProperties(
        run,
        context
    ) {
        const directRPr =
    ...
    function runToHtml(
        ...
        return (
            `<span style="${escapeAttribute(css)}">` +
            content +
            "</span>"
        );
    }
    ...
    function buildParagraphDescriptor(
        paragraph,
        baseContext
    ) {
    ...
    function tableCellToHtml(
         ...
         * EZ A FONTOS RÉSZ:
         *
         * a cella lesz a kép aktuális
         * szélességi referenciája.
         */
        const cellContext = {
            ...context,
    ...
    function tableToHtml(
        ...
        for (
            ...
            /*
             * Soronként eldöntjük, hogy:
             *
             * szélesség vagy magasság
             * legyen a kép referenciája.
             */
            const rowAnalysis =
                analyzeTableRow(
                    row
    ...
    const SAFE_INHERITED_PROPERTIES =
        new Set([
            "color",
            "font-family",
            "font-size",
    ...
    function getInheritedInlineValue(
        element,
        property
    ) {
        let parent =
    ...
    function removeInheritedSpanStyles(
        root
    ) {
        const spans = [
    ...
    async function convertDocxToHtml(
        ...
        if (!documentFile) {
            throw new Error(
                "Ez nem érvényes DOCX fájl."
            );
        }

        const documentXmlText =
        ...
        progress?.(
            "Word stílusok feldolgozása..."
        );

        const themeFonts =
            await readThemeFonts(
                zip
            );
        ...
        if (!body) {
            throw new Error(
                "Nem található a dokumentum tartalma."
            );
        }
